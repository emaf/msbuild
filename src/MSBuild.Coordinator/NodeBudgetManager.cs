...
{
    private const int PriorityAgingBypassThreshold = 3;
    ...
    public NodeBudgetManager(int totalBudget, int highPriorityReservedNodes = 0, int maxNodesPerBuild = 0)
    {
    ...
    public int TryGrant(BuildGrant grant)
    {
        ...
        lock (_lock)
        {
            ...
            if (hasWaiters && ShouldQueueBehindWaiters_NoLock(grant.Priority))
            {
    ...
    private bool ShouldQueueBehindWaiters_NoLock(CoordinatorBuildPriority priority)
    {
        if (priority == CoordinatorBuildPriority.High)
        {
            // Aged Low/Normal waiters can have High effective priority, but they still cannot
            // consume high-priority reserved capacity. Let real High arrivals compare only
            // against raw High waiters so reserved capacity is not stranded behind aged work.
            return priority <= _highestWaiterRawPriority;
        }

        return priority <= GetHighestEffectivePriority_NoLock();
    }
    ...
    private static CoordinatorBuildPriority GetEffectivePriority(BuildGrant grant)
        => (CoordinatorBuildPriority)Math.Min(
            (int)CoordinatorBuildPriority.High,
            (int)grant.Priority + (grant.BypassCount / PriorityAgingBypassThreshold));
}
