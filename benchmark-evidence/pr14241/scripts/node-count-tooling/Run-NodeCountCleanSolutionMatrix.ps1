<#
.SYNOPSIS
Runs balanced no-Coordinator clean-solution node-count benchmark blocks.
...
param(
    ...
    [string[]]$ConditionKeys = @('N4', 'N8', 'N16'),
    [int]$WarmupBlocks = 1,
...
function Invoke-PrepareRepository {
    ...
    $parameters = @{
        ...
        NoCoordinatorNodeCount = $NodeCount
        Prepare = $true
        SkipWarm = $SkipWarm
    }
...
function Reset-CleanWorktrees {
    ...
    foreach ($worktree in @(Get-WorktreeCommitRecords -Repository $Repository)) {
        & git -C $worktree.Path reset --hard $Repository.ExpectedCommit | Out-Host
        if ($LASTEXITCODE -ne 0) {
            throw "Resetting '$($worktree.Path)' to '$($Repository.ExpectedCommit)' failed with exit code $LASTEXITCODE."
        }
...
function Test-ScenarioResult {
    ...
    if (Test-Path -LiteralPath $processPath) {
        $coordinatorProcesses = @(Import-Csv -LiteralPath $processPath | Where-Object name -eq 'MSBuild.Coordinator.exe')
        if ($coordinatorProcesses.Count -ne 0) {
            $errors.Add("Observed $($coordinatorProcesses.Count) Coordinator process sample(s) in a no-Coordinator scenario.")
        }
    }
    foreach ($run in $runs) {
        if ((Test-Path -LiteralPath $run.stdout) -and
            (Get-Content -LiteralPath $run.stdout -Raw).Contains('CoordinatorNodeGrantReceived', [StringComparison]::Ordinal)) {
            $errors.Add("Build '$($run.label)' logged an unexpected Coordinator node grant.")
        }
    }
...
function Invoke-Condition {
    ...
    $metadata = [ordered]@{
        ...
        DesignKind = 'single-build-node-count'
    ...
    try {
        if ($Workload.EndsWith('-clean', [StringComparison]::Ordinal)) {
            ...
            Invoke-PrepareRepository -Repository $Repository -Bootstrap $bootstrap -SetupRoot (Join-Path $scenarioRoot 'setup') -SkipWarm $true -NodeCount $Definition.NodeBudget
            $setupWorktreeStatus = @(
                foreach ($worktree in @(Get-WorktreeCommitRecords -Repository $Repository)) {
                    $status = @(& git -C $worktree.Path status --porcelain)
                    if ($LASTEXITCODE -ne 0) {
                        throw "Reading status for '$($worktree.Path)' failed with exit code $LASTEXITCODE."
                    }
                    [pscustomobject]@{
                        Name = $worktree.Name
                        Path = $worktree.Path
            ...
            $metadata.SetupWorktreeStatus = $setupWorktreeStatus
            if (@($setupWorktreeStatus | Where-Object { $_.Status.Count -ne 0 }).Count -ne 0) {
                throw 'A benchmark worktree was dirty after reset, clean, and restore.'
            }
        }
        ...
        $parameters = @{
            ...
            NodeBudget = $Definition.NodeBudget
            NoCoordinatorNodeCount = $Definition.NodeBudget
            Slice = $Definition.Slice
            Reservation = $Definition.Reservation
...
$conditionDefinitions = [ordered]@{
    'N4' = [pscustomobject]@{
        ...
        Description = 'Candidate binary without Coordinator at /m:4.'
    }
    'N8' = [pscustomobject]@{
        Key = 'N8'
        BootstrapRole = 'candidate'
        ...
        Reservation = 0
        CandidatePriority = 'Normal'
        Description = 'Candidate binary without Coordinator at /m:8.'
    }
    'N16' = [pscustomobject]@{
        Key = 'N16'
        BootstrapRole = 'candidate'
        ...
        Reservation = 0
        CandidatePriority = 'Normal'
        Description = 'Candidate binary without Coordinator at /m:16.'
    }
}
