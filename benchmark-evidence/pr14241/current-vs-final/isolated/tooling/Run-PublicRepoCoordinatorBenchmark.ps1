...
param(
    ...
    [int]$Slice = 4,
    [int]$Reservation = 4,
    [switch]$AutomaticPolicy,
    [switch]$OmitPriorityEnvironment,
...
function New-BenchmarkProcess {
    ...
    if ($UseCoordinator) {
        ...
        if (-not $OmitPriorityEnvironment) {
...
function Invoke-Scenario {
    ...
    [pscustomobject]@{
        ...
        slice = $Slice
        reservation = $Reservation
        automaticPolicy = [bool]$AutomaticPolicy
...
Write-Host "AutomaticPolicy: $([bool]$AutomaticPolicy)"
