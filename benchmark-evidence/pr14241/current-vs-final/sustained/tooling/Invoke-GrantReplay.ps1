...
$stagingMetadataPath =
    Join-Path (Split-Path -Parent $BootstrapRoot) 'staging-metadata.json'
if (Test-Path -LiteralPath $stagingMetadataPath -PathType Leaf) {
    $stagingMetadata =
        Get-Content -LiteralPath $stagingMetadataPath -Raw |
        ConvertFrom-Json
    $role = [string]$stagingMetadata.Role
    $expectedCommit = [string]$stagingMetadata.ExpectedCommit
}
else {
    $role = if ($productVersion.Contains(
    ...
    $expectedCommit = if ($role -eq 'final') {
        $definition.Final.Commit
    }
    else {
