...
function Test-SustainedGrantPolicyEvidence {
    ...
    $finalGrantMetrics = if ($Condition -eq 'BASE') {
        @()
    }
    else {
        @($GrantMetrics)
    ...
    $idleEightNodeGrantEvents = @(
        $Trace.Events |
            Where-Object {
                ...
                [int]$_.QueueDepth -eq 0
            }
    )
    if ($Condition -ne 'BASE' -and $oversizedFinalGrants.Count -gt 0) {
        $errors.Add("FINAL replay contains $($oversizedFinalGrants.Count) grant(s) above four nodes.")
    }
    if ($Condition -ne 'BASE' -and $idleEightNodeGrantEvents.Count -gt 0) {
        $errors.Add("FINAL trace contains $($idleEightNodeGrantEvents.Count) idle eight-node grant(s).")
    }
